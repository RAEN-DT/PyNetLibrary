    ...
    if len(parts) < 4:
        ...
        if valid_codes and parts[2] not in valid_codes:
            ...
            if cv and exp_n and cv != exp_n:
                errs.append(f'Inconsistencia: param PyNET_3D_Clasificacion="{cv}" pero codigo en nombre "{parts[2]}"="{exp_n}".')
    return errs
...
for bic_str in matrix:
    ...
    try:
        ...
        for et in types:
            clas_param_val = None
            cp = et.LookupParameter('PyNET_3D_Clasificacion')
