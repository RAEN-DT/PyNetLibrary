...
> **CRITICAL — always use ABSOLUTE paths** for `script_Path` in `deploy_script_button` and `update_script_button`. Relative paths save silently but fail at runtime with "Script File not found". Example: `C:\Users\<user>\Documents\PyNet\Scripts\MyScript.py`
